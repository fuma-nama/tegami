...
Composer packages publish through git tags, so the Composer plugin requires the [git](/plugins/git) plugin. The [GitHub](/plugins/github) and [GitLab](/plugins/gitlab) plugins include `git()` already.
...
<Callout type='info' title="Monorepos and Packagist">

Public Packagist indexes one package per repository. A monorepo package will not appear on packagist.org from a shared repo. Use repository splitting or a private registry such as [Private Packagist](https://packagist.com) or [Satis](https://github.com/composer/satis). Tegami still manages versions, constraints, and tags either way.
