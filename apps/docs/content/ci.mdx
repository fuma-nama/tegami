...
Use `tegami ci-pr` on pull requests to show a release preview in CI. It renders markdown that:
...
Write the preview to the GitHub Actions job summary. It renders as markdown on the workflow run page.
...
`tegami ci-pr` reads pull request metadata from `GITHUB_EVENT_PATH` when run in GitHub Actions. Pass `--print` to write raw markdown (for example to `$GITHUB_STEP_SUMMARY`). Without `--print`, the preview is shown in the terminal with formatted output.
