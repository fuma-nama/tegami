...
Set `pull-requests: write` so the [GitHub plugin](/plugins/github) can open _Version Packages_ pull requests. The same `GITHUB_TOKEN` is also used to create GitHub releases after publish.
...
To run this flow on more than one branch, e.g. a canary channel, or maintenance branches for older majors, see [Multiple Release Lines](/multi-version).
...
```yaml title=".github/workflows/tegami-pr.yml" tab="Generate Comment"
name: Tegami PR
...
```yaml title=".github/workflows/tegami-pr-comment.yml" tab="Post Comment"
...
Only "Post Comment" needs `pull-requests: write`, and it checks out your default branch instead of the pull request head, so pull requests from forks are commented on without running their code with a writable token.

## Custom CLI hooks

`runCli` accepts optional hooks to replace the default `draft()` or `publish()` behavior:
