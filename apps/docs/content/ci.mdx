...
Use `tegami ci-pr` on pull requests to replace the Changesets bot. It posts (or updates) a comment that:

1. Introduces Tegami and tells contributors to add changelog files.
2. Links to create a new changelog file on GitHub.
3. Shows a release preview from pending changelogs, including packages bumped in the PR.
...
`tegami ci-pr` reads pull request metadata from `GITHUB_EVENT_PATH` when run in GitHub Actions. Use `--print` locally to preview the comment body without posting.

Requires the `gh` CLI and `GITHUB_TOKEN` with permission to comment on pull requests.
