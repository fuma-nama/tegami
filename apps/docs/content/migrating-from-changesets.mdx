...
- `tegami publish` — publish from the applied plan in `.tegami/publish-plan`.
...
Run `tegami ci` on pushes to your main branch. It versions when pending changelogs exist; otherwise it publishes from the publish plan.
...
Because the publish plan lives in git, failed publish jobs can be retried safely. See [CI Setup](/ci) for build steps and other details.
...
Run `tegami ci-pr` on pull requests. It writes a release preview to the GitHub Actions job summary, with a link to create a changelog file and contributor guidance. See [CI Setup — Pull request preview](/ci#pull-request-preview) for the workflow example.

```yaml title=".github/workflows/tegami-pr.yml"
name: Tegami PR
  ...
  preview:
    ...
    steps:
      ...
      - name: Release preview
        run: pnpm tegami ci-pr --print >> "$GITHUB_STEP_SUMMARY"
```
...
| `baseBranch`               | `github({ cli: { versionPr: { base: "main" } } })`        |
...
- **Cross-registry monorepos** — npm and Cargo in one graph ([npm](/plugins/npm), [cargo](/plugins/cargo)).
- **Configurable dependency bump policy** — especially for `workspace:` peer dependencies ([npm plugin](/plugins/npm#dependency-bumps)).
- **Publish plan in git** — `.tegami/publish-plan` makes failed publishes safe to retry ([CI Setup](/ci)).
...
4. Update CI to run `tegami ci` on main and `tegami ci-pr` on pull requests.
