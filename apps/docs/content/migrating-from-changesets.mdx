...
Run `tegami ci-pr` on pull requests. It posts (or updates) a comment with a release preview, a link to create a changelog file, and contributor guidance.
...
jobs:
  comment:
    ...
    steps:
      ...
      - run: pnpm tegami ci-pr
        env:
          GITHUB_TOKEN: ${{ github.token }}
```

See [CI Setup — Pull request comments](/ci#pull-request-comments) for details.
