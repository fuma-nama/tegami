...
There is nothing to enter or exit: adding `prerelease` puts a package on the prerelease line, and removing it graduates the package to a stable release. See [Multiple Release Lines](/multi-version) for a canary branch and maintenance branches running side by side.
