...
Workspaces with a single package skip the package selector, choose `auto-generate from commits` in the release type prompt instead.
...
1. On the first release, Tegami applies the bump and writes the notes to `CHANGELOG.md`. The file stays in `.tegami/` as replay-only.
2. When `tegami` later releases `tegami@1.1.0`, the same notes are appended again and the file is removed.

<Callout title="Replay-only Entry">
